...
def creerCarte(matricule, solde, activee):
	try:
		curseur = getConnexionBD().cursor()
		requete = '''
		...
		'''
		curseur.execute(requete, (matricule, solde, activee)) 
		connexionBD.commit()
		nbTuplesTraites = curseur.rowcount
		curseur.close()
		return nbTuplesTraites
...
def historique_reservations(numeroCarte):
    try:
        curseur = getConnexionBD().cursor()
        requete = '''
            SELECT dateResa
        ...
        curseur.execute(requete, (numeroCarte,))
        enregistrements = curseur.fetchall() 
        dateHistorique = [enregistrement[0].strftime('%Y-%m-%d') for enregistrement in enregistrements]
        curseur.close()
