...
def connexionGestionnaire():	
	return render_template( 'vueGestionnaireAccueil.html' ,echecConnexion = False , saisieIncomplete = False )
...
@app.route('/gestionnaire/Personnel/AvecCarte' , methods = ['GET','POST'])
...
@app.route('/gestionnaire/Personnel/AvecCarte/crediter/<numeroCarte>' , methods = ['POST','GET'])
def crediterCarte(numeroCarte):
	...
	if somme is not None and somme.isdigit():
		...
		return redirect('/gestionnaire/Personnel/AvecCarte/<numeroCarte>')
	else:
		return "Erreur : Somme invalide."
	
@app.route('/gestionnaire/Personnel/AvecCarte/historique/<numeroCarte>',methods = ['GET'])
def historique(numeroCarte):
	reservations = modeleResanet.historique_reservations(numeroCarte)
	return render_template('vueHistorique.html', reservations = reservations)
	
@app.route('/gestionnaire/Carte/Créer/', methods=['GET', 'POST'])
def creerCarte():
	if request.method == 'POST':
		...
		success = modeleResanet.creerCarte(matricule, solde, activee)

		return redirect('/gestionnaire/Personnel/AvecCarte')
	else:
		matricule = request.args.get('matricule', '')
		return render_template('vueCréerCarte.html', matricule=matricule)

	
@app.route('/gestionnaire/Reservations/date/Form', methods=['GET'])
def resa_date():
...
@app.route('/gestionnaire/Reservations/carte/Form' , methods = ['GET'])
def ResaCarte():
...
def afficher_reservations_par_carte():
	carte_resa_str = request.form.get('carte_resa')
	reservations = modeleResanet.historique_reservations(carte_resa_str)
